...
var similarWizardTemplate = document.querySelector('#similar-wizard-template') // Найдём шаблон, который мы будем копировать. //а иначе было бы просто var similarWizardTemplate = document.querySelector('.setup-similar-item');
    .content
...
var wizards = [
  {
    ...
    eyeColor: EYES_COLOR[GenerateRundomIndex(EYES_COLOR)]
  },
  {
    name: NAMES_WIZARD[GenerateRundomIndex(NAMES_WIZARD)]+" "+SURNAMES_WIZARD[GenerateRundomIndex(SURNAMES_WIZARD)],
    coatColor: COATS_COLOR[GenerateRundomIndex(COATS_COLOR)],
...
var renderWizard = function (wizard) { // функция добавляет волшебнику что-то
  ...
  wizardElement.querySelector('.wizard-eyes').style.fill = wizard.eyeColor; // меняет цвет глаз
